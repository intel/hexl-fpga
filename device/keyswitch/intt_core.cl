...
void _intt_internal(channel ulong4 ch_intt_modulus,
  ...
  while (true) {
    ...
    unsigned roots_acc = FPGA_NTT_SIZE * (key_modulus_idx - key_modulus_start);
    ...
    for (unsigned m = (n >> 1); m >= 1; m >>= 1) {
      ...
      for (unsigned k = 0; k < FPGA_NTT_SIZE / 2 / VEC; k++) {
        ...
        if (b_first_stage) {
          intt_elements elements = read_channel_intel(ch_intt_elements_in);
          #pragma unroll
        ...
        for (unsigned n = 0; n < VEC; n++) {
          unsigned i = (k * VEC + n) >> logt;    // i is the index of groups
        ...
        *(uint_vec_t*)cur_roots =
        ...
        for (int n = 0; n < VEC; n++) {
          ...
          curX[VEC + n] = MultiplyUIntMod(SubUIntMod(x_j1, x_j2, prime), W_op,
                                          prime, prime_k);
