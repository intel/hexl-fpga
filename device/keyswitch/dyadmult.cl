...
#pragma ivdep
    while (true) {
        if (l == 0) {
            ...
            if (ntt_ins == 0 && j == 0) {
#pragma unroll
                for (int key_component = 0;
                     key_component < MAX_KEY_COMPONENT_SIZE; key_component++) {
                    write_channel_intel(ch_intt_modulus[1 + key_component],
                                        last_moduli);
                }
            }
        }
...
#pragma unroll
        for (unsigned k = 0; k < key_component_count; ++k) {
            ...
            uint64_t prod = MultiplyUIntMod(val, ((ulong*)&keys)[k],
                                            curr_moduli.s0, curr_moduli.s3);
