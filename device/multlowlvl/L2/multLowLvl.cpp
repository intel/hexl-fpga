...
void LaunchINTT1(std::vector<uint64_t> &primes) {
  // launch iNTT
  launch_intt(L1::helib::bgv::GetINTT1(), primes, COEFF_COUNT);
}

void LaunchINTT2(std::vector<uint64_t> &primes) {
  // launch iNTT
  launch_intt(L1::helib::bgv::GetINTT2(), primes, COEFF_COUNT);
}
...
void LaunchBringToSet(std::vector<uint8_t> &pi_primes_index,
  ...
  if (engine == 1) {
    Timer timer("L1::helib::bgv::BringToSet");
    queue_copy(*ctxt.q_scale1, scale_param_set, scale_param_set_buf);
    L1::helib::bgv::BringToSet(*ctxt.q_scale1, COEFF_COUNT,
                               *scale_param_set_buf, P, Q, I, plainText);
  } else {
    Timer timer("L1::helib::bgv::BringToSet2");
    queue_copy(*ctxt.q_scale2, scale_param_set, scale_param_set_buf);
    L1::helib::bgv::BringToSet2(*ctxt.q_scale2, COEFF_COUNT,
                                *scale_param_set_buf, P, Q, I, plainText);
  }
}
...
void TensorProduct(std::vector<uint64_t> &primes,
  ...
  L1::helib::bgv::TensorProduct(q, *primes_mulmod_buf);
}
...
void Load(std::vector<uint64_t> &input, std::vector<uint8_t> &primes_index) {
  ...
  assert(engine == 1 || engine == 2);
  if (engine == 1) {
    L1::helib::bgv::BringToSetLoad(*ctxt.q_load1, copy_event, *input_buff,
                                   *primes_index_buf);
  } else {
    L1::helib::bgv::BringToSetLoad2(*ctxt.q_load2, copy_event, *input_buff,
                                    *primes_index_buf);
  }
}
...
void Store(std::vector<uint64_t> &output1, std::vector<uint64_t> &output2,
  ...
  auto kernel_store0_event =
      L1::helib::bgv::TensorProductStore0(q_tensor_product_store0, output1_buf);
  ...
  kernel_event[0] = L1::helib::bgv::TensorProductStore12(
  ...
  for (size_t i = 1; i < iters + 1; i++) {
    ...
    size_t last = (i - 1) % 2;
    if (i < iters) {
      kernel_event[j] = L1::helib::bgv::TensorProductStore12(
...
void MultLowLvl(std::vector<uint64_t> &a0, std::vector<uint64_t> &a1,
  ...
  L2::helib::bgv::LaunchBringToSet1(a_primes_index, output_primes_index,
                                    plainText, pi_reorder_primes_index1);
  L2::helib::bgv::LaunchBringToSet2(b_primes_index, output_primes_index,
                                    plainText, pi_reorder_primes_index2);

  // load part 1
  L2::helib::bgv::Load1(a0, pi_reorder_primes_index1);
  L2::helib::bgv::Load2(b0, pi_reorder_primes_index2);

  L2::helib::bgv::TensorProduct(ctxt.primes, output_primes_index);

  // load part 2
  L2::helib::bgv::Load1(a1, pi_reorder_primes_index1);
  L2::helib::bgv::Load2(b1, pi_reorder_primes_index2);

  // launch store
  L2::helib::bgv::Store(c0, c1, c2, 0);
}
}  // namespace bgv
}  // namespace helib
}  // namespace L2
