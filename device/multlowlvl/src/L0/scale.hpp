...
event scale(sycl::queue &q, uint32_t coeff_count,
  ...
  event e = q.submit([&](handler &h) {
    ...
    h.single_task<KernelName>([=]() [[intel::kernel_args_restrict]] {
      ...
      ulong c[MAX_COEFF_COUNT][MAX_P_BANKS];
      ...
      ASSERT(scale_param_set_acc.size() == scale_param_set_size, "mismatch");
      for (int i = 0; i < scale_param_set_acc.size(); i++) {
      ...
      [[intel::disable_loop_pipelining]] for (int poly = 0; poly < 2; poly++) {
        ...
        [[intel::disable_loop_pipelining]] for (uint i = 0; i < P; i++) {
          for (uint n = 0; n < coeff_count; n++) {
            // ulong c_i = acc_c[i * coeff_count + n];
        ...
        [[intel::disable_loop_pipelining]] for (uint32_t j = 0; j < Q; j++) {
          ...
          for (uint32_t n = 0; n < coeff_count; n++) {
            uint64_t part1 = 0;
            int k = ks[n];
...
#pragma unroll
            ...
            bool debug_case = (j == 0 && n == 18025) ||
                              (j == 0 && n == 33392) ||
                              (j == 0 && n == 39979) || (j == 0 && n == 42551);
            if (debug_case) {
              PRINTF("==========DEBUG: k = %d, t = %d, v = %f, n = %d\n", k, t,
                     v[n], n);
              PRINTF("==========DEBUG: k = %d, t = %d, v = %d\n", k, t,
                     round_v);
            }
            ...
            if (out == 862941756731727L) {
              PRINTF("==========DEBUG: j = %d, n = %d\n", j, n);
            }
