...
event ntt(sycl::queue &q, const std::vector<ulong4> &config) {
  ...
  auto e = q.submit([&](handler &h) {
    ...
    h.single_task<tt_kernelNameClass>([=]() {
      ...
      [[intel::disable_loop_pipelining]] while (true) {
        ...
        ulong4 cur_moduli = acc_config[prime_index];
        ...
        ASSERT(VEC >= 2 && VEC <= 16, "Not supported VEC");
        PRINTF("fpga_ntt_size = %llu, modulus = %llu\n", fpga_ntt_size,
               modulus);
