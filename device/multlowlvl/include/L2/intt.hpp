...
void launch_intt(intt_t &intt, const std::vector<uint64_t> &primes,
  ...
  std::cout << "intt old b_initialized address: " << &b_initialized << std::endl;
  ...
  if (!b_initialized) {
    ...
    launch_intt_config_tf<intt_t, id>(intt, degree, primes);
    ...
    launch_compute_inverse<intt_t, id>(intt, degree, primes);
  }
}
...
template <int id>
void launch_intt_config_tf(INTT_Method &intt, unsigned long degree,
                           const std::vector<uint64_t> &primes) {
  static std::vector<uint64_t> invRootOfUnityPowers;
...
template <int id>
void launch_compute_inverse(INTT_Method &intt, unsigned long degree,
                            const std::vector<uint64_t> &primes) {
  // generate intt configurations
  static std::vector<sycl::ulong4> intt_configs;
  for (uint64_t prime : primes) {
...
template <int id>
void launch_intt(INTT_Method &intt, const std::vector<uint64_t> &primes,
                 uint64_t degree) {
  // Create queue, get platform and device
#if defined(FPGA_EMULATOR)
  sycl::ext::intel::fpga_emulator_selector device_selector;
  ...
  sycl::event e;
  static bool b_initialized = false;
  //std::cout << "intt new b_initialized address: " << &b_initialized << std::endl;
  ...
  if (!b_initialized) {
    ...
    // config the twiddle factor factory kernel
    launch_intt_config_tf<id>(intt, degree, primes);
    ...
    launch_compute_inverse<id>(intt, degree, primes);
  }
}
