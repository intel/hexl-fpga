...
using pipe_tensor_product_ready =
    ext::intel::pipe<class TensorProductReady, uint64_t>;
using pipe_break_into_digits_ready =
    ext::intel::pipe<class BreakIntoDigitsReady, uint64_t>;
using pipe_key_switch_digits_ready =
    ext::intel::pipe<class KeySwitchDigitsReady, uint64_t>;

using pipe_data_ptr_c01 = ext::intel::pipe<class DataPtrC01, uint64_t>;
